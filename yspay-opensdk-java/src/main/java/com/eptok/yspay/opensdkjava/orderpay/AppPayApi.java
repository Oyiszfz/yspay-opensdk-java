...
public class AppPayApi {
     ...
     * @throws Exception
     */
    public static String sdkpay(OnlineReqDataVo vo) throws Exception {
        /** 1、获取业务参数 */
        String biz_content = JSONObject.toJSONString(vo.getParamData());
        logger.info("AppPayApi-sdkpay-bizContent:"+biz_content);

        /** 2、组装入参的数据*/
        Map<String, String> params = new HashMap<>();
        //接口名称
        params.put("method","ysepay.online.sdkpay");
        //商户在银盛支付平台开设的用户号[商户号]
     ...
     * @throws Exception
     */
    public static String mobileControlsPay(OnlineReqDataVo vo) throws Exception {
        /** 1、获取业务参数 */
        String biz_content = JSONObject.toJSONString(vo.getParamData());
        logger.info("AppPayApi-mobileControlsPay-bizContent:"+biz_content);

        /** 2、组装入参的数据*/
        Map<String, String> params = new HashMap<>();
        //接口名称
        params.put("method","ysepay.online.mobile.controls.pay");
        //商户在银盛支付平台开设的用户号[商户号]
        params.put("partner_id",vo.getPartnerId());
        //发送请求的时间，格式"yyyy-MM-dd HH:mm:ss"
        params.put("timestamp", DateUtil.getDateNow());
        ...
        /** 3、对数据进行加签，并存入请求数据*/
        String sign = YsOnlineSignUtils.sign(params, vo.getPrivateKeyPassword(),vo.getPrivateKeyFilePath());
        logger.info("AppPayApi-mobileControlsPay-signData:"+sign);
        params.put("sign",sign);

        /**4、发送请求*/
        String result = HttpClientUtil.sendPostParam(vo.getReqUrl(), StringUtil.mapToString(params));
        logger.info("AppPayApi-mobileControlsPay-remoteResultData:"+result);

        /**5、对接口回执进行验签*/
        JSONObject resultJson = JSONObject.parseObject(result, Feature.OrderedField);
        //加签内容
        String content = resultJson.getString("ysepay_online_mobile_controls_pay_response");
        //加签值
        String sinaValue = resultJson.getString("sign");
        boolean flag = YsOnlineSignUtils.rsaCheckContent(content,sinaValue,Constants.CHARSET_GBK,vo.getYsPublicKeyFilePath());
        if(!flag){
